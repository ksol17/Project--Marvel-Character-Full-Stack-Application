...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:Preciosa2016!@localhost/marvel'
...
def create_database():
    root_engine = create_engine("mysql+mysqlconnector://root:Preciosa2016!@localhost")  # No database specified
    with root_engine.connect() as connection:
        connection.execute(text("CREATE DATABASE IF NOT EXISTS marvel"))
        connection.execute(text("USE marvel"))
...
def create_character():
    try:
        ...
        return jsonify({"error": "Invalid data", "message": e.messages}), 400  # Better error response
